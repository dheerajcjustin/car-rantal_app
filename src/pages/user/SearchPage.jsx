...
const SearchPage = () => {
      ...
      const getKey = (pageIndex, previousPageData) => {
            return `/search${location.search}&page=${
                  pageIndex + 1
            }&filter=${selectPickup}`; // SWR key
      };
      ...
      return (
            <div className="bg-[#FDD23F] ">
                  ...
                  {data && (
                        <FilterMobile
                              time={data[0]?.time}
                              pickups={data[0]?.pickups}
                        />
                  )}
                  {isLoading && <FilterMobile />}
                  ...
                  <div className="grid md:grid-cols-4 grid-cols-3 gap-5">
                        {data && (
                              <FilterDesktop
                                    time={data[0]?.time}
                                    pickups={data[0]?.pickups}
                                    setSelectedPickups={setSelectPickup}
                              />
                        )}
                        {isLoading && <FilterDesktop />}
