...
const VendorBookingCard = ({ data, mutate, setLoading }) => {
      const pickupCompleted = async (order) => {
            try {
                  ...
                  const res = await authInstance.patch(
                        "/vendor/bookingsStatus",
                        { orderId: order._id, pickupStatus: "pickupStatus" },
                  );
      ...
      const DropOffCompleted = async (order) => {
            try {
                  ...
                  const res = await authInstance.patch(
                        "/vendor/bookingsStatus",
                        { orderId: order._id, dropOffStatus: "dropOffStatus" },
                  );
            ...
            <div className="flex flex-row justify-around bg-gray-200 w-full rounded-3xl pt-3 shadow-lg shadow-gray-500/50     ">
                  ...
                  <div className="text-center  my-auto">
                        <h6 className="sm:text-xl">
                              {!data?.myOrders?.pickupStatus
                                    ? data?.myOrders?.pickupTime
                                    : data.myOrders?.dropOffTime}
                        </h6>
                        <h6 className="sm:text-xl">
                              {" "}
                              {!data?.myOrders?.pickupStatus
                                    ? data?.myOrders?.pickupDate
                                    : data.myOrders?.dropOffDate}{" "}
                        </h6>
                        <h1 className="sm:text-xl">
                              {data?.myOrders?.pickup?.name}{" "}
                        </h1>
                        <h1 className="sm:text-xl">
                              Price: {data?.myOrders?.price}{" "}
                        </h1>
                  </div>
                  <div className="h-full flex flex-col justify-center">
                        {data?.myOrders?.dropOffStatus ? (
                              ...
                              <button
                                    className="bg-green-400 text-xl p-5 rounded-xl"
                                    onClick={() =>
                                          DropOffCompleted(data?.myOrders)
                                    }
                              >
                                    {" "}
                                    Drop Off
