...
const EditCarFields = ({ valueSetting, carData, setImageUpload, setCarData }) => {
  ...
  const selectService = (e) => {
    const selected = pickupPointList.filter(pickup => pickup._id == e.target.value)[0];
  ...
  return (
    <div className="grid sm:grid-cols-2  grid-cols-1  gap-5   bg-[#FDD23F]">
      <div className="w-full ml-6 relative">
        ...
        <label
          htmlFor="modelName"
          className={`absolute left-32 top-[30%] peer-focus:left-16 peer-focus:top-[-10%] bg-white peer-focus:bg-[#FDD23F]  peer-focus:opacity-100  peer-focus:text-sm  rounded-lg px-2 ${carData.modelName ? "opacity-0" : "opacity-50"
      ...
      <div className="w-full ml-6 relative">
        ...
        <label
          htmlFor="price"
          className={`absolute left-32 top-[30%] peer-focus:left-16 peer-focus:top-[-10%] bg-white peer-focus:bg-[#FDD23F] peer-focus:opacity-100  peer-focus:text-sm  rounded-lg px-2  text-3xl transition-all duration-200 ${carData.price ? "opacity-0" : "opacity-50"
            }`}
        >
          Price
        </label>
      ...
      <div className="w-full ml-6 relative">
        ...
        <label
          htmlFor="rcNumber"
          className={`absolute left-32 top-[30%] peer-focus:left-16 peer-focus:top-[-10%] bg-white peer-focus:bg-[#FDD23F] peer-focus:opacity-100  peer-focus:text-sm  rounded-lg px-2   text-3xl transition-all duration-200  ${carData.rcNumber ? "opacity-0" : "opacity-50"
            }`}
        >
          Rc Number
        </label>
      ...
      <div className='w-[90%]  border-2 border-black rounded-3xl text-center flex flex-col items-center justify-center break-words'>
        <div className=' break-words'>{selectedPickup.join(' , ')}</div>
        ...
        <select
          ...
          <option value="#">Select Pickup</option>\
          {
            ...
            pickupPointList &&
            pickupPointList.map(pickupPoint => <option key={pickupPoint._id} value={pickupPoint._id}>{pickupPoint.name}</option>)
          }
