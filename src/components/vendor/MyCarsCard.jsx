...
const MyCarsCard = ({ photos, title, RC, price, seatNum, location, status, transmission, fuelType, pickupPoint, documents }) => {
  ...
  return (
    <>
      {/**/}
      {editCarModal && <EditCar onClose={onClose} RC={RC} title={title} price={price} seatNum={seatNum} location={location} photos={photos} transmission={transmission} fuelType={fuelType} documents={documents} pickupPoint={pickupPoint} />}
      <div className="  min-w-fit  rounded-3xl bg-[#10191F] pt-3 shadow-lg shadow-gray-500/50      ">
        <div className="flex flex-col  align-middle items-center w-full">
          <h2 className="text-xl font-semibold  text-cyan-50">{title}</h2>
          <img
            src={image}
            alt="photo of amg"
