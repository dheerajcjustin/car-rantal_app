...
const MyEvents = ({ userType }) => {
      const { data, isLoading, error } = useSWR(
            `${(userType = "user" ? `/bookings` : `/vendor/bookings`)}`,
            fetcher,
      );
      ...
      return (
            <div>
                  ...
                  <div className="grid grid-cols-2 border-banana">
                        <h1
                              className={`text-center  border-banana rounded-xl shadow-lg ${
                                    showCompleted
                                          ? ``
                        ...
                        <h1
                              className={`text-center bg border-banana rounded-xl  shadow-lg ${
                                    showCompleted
                  ...
                  {isLoading &&
                        arr.map((elm) => <UserBookingCardSkelton key={elm} />)}
                  {data && (
                        <>
                              {showCompleted ? (
                                    <EventList
